...
struct dynarray* dynarray_create() {
  struct dynarray* n_arr = malloc(sizeof(struct dynarray));

  n_arr->size = 0, n_arr->capacity = 2, n_arr->data = malloc(n_arr->capacity * sizeof(void*));
  return n_arr;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  if (da->size == da->capacity){
    da->data = realloc(da->data, sizeof(void*) * (da->capacity*2));
    da->capacity *= 2;
  }
  int last_pos = da->size;
  da->data[last_pos] = val;
  da->size += 1;
  return;
}
