...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    new_data[i] = da->data[(da->head + i) % da->capacity];
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  da->data[(da->head+da->size)%da->capacity] = val;
  da->size++;
}
...
void* front_value(struct dynarray* da){
  return da->data[da->head];
}

void* dequeue(struct dynarray* da){
  void* head = front_value(da);
  da->head = (da->head+1)% da->capacity;
  da->size--;
  return head;
}

int queue_check(struct dynarray* da){
  if (da->size == 0){
    return 1;
  }
  else return 0;
